...
const FISH_PARALLAX_DEPTH_BASE = 36;
const FISH_PARALLAX_DEPTH_TOP_SCALE = 196;
...
const CANVAS_FISH_ZONE_STYLE = Object.freeze({
	sunlight: { opacity: 0.9, brighten: 0.1 },
	twilight: { opacity: 0.92, brighten: 0.21 },
	midnight: { opacity: 0.88, brighten: 0.19 },
	abyss: { opacity: 0.74, brighten: 0.2 },
});
const FISH_PATH_2D = typeof Path2D === "function" ? new Path2D(FISH_ICON_PATH) : null;
...
const SCHOOL_BLUEPRINTS = [
	...
	{ id: "twilight-a", zone: "twilight", y: 44, direction: "left", count: 7, size: 9 },
	{ id: "twilight-b", zone: "twilight", y: 51, direction: "right", count: 7, size: 9 },
	{ id: "midnight-a", zone: "midnight", y: 63, direction: "right", count: 6, size: 9 },
	{ id: "midnight-b", zone: "midnight", y: 70, direction: "left", count: 6, size: 9 },
	{ id: "abyss-a", zone: "abyss", y: 79, direction: "left", count: 5, size: 9 },
	{ id: "abyss-b", zone: "abyss", y: 86, direction: "right", count: 5, size: 9 },
	{
		id: "seafloor-a",
		zone: "abyss",
		y: 93,
...
const brightenHexColor = (hex, brightenAmount = 0) => {
	...
	const mix = (channel) => {
		...
		return Math.round(value + (255 - value) * clamp(brightenAmount, 0, 1));
	};
...
const getFishParallaxShift = (fish, depthValue, scrollValue, viewportHeight) => {
	...
	return clamp(depthShift + microScrollShift, -FISH_SCROLL_PARALLAX_MAX_SHIFT, FISH_SCROLL_PARALLAX_MAX_SHIFT);
};
...
const getFrameIntervalMs = () => {
	...
	const overriddenFps = Number.isFinite(fpsFromQuery) ? fpsFromQuery : Number.isFinite(fpsFromStorage) ? fpsFromStorage : null;
	...
	const isHighEndDesktop = !isSmallScreen && !isCoarsePointer && hardwareThreads >= 8 && (deviceMemory === 0 || deviceMemory >= 8);
	...
	const targetFps = clamp(overriddenFps ?? defaultFps, MIN_TARGET_FPS, MAX_TARGET_FPS);

	return 1000 / targetFps;
};

const getFishDensityProfile = () => {
	if (typeof window === "undefined") {
		return {
			baseDensity: DESKTOP_FISH_DENSITY,
			zoneMultipliers: DESKTOP_ZONE_DENSITY_MULTIPLIERS,
		};
	}
	...
	if (isLowPowerCpu || isLowMemoryDevice) {
		return {
			baseDensity: LOW_POWER_MOBILE_FISH_DENSITY,
			zoneMultipliers: LOW_POWER_MOBILE_ZONE_DENSITY_MULTIPLIERS,
		};
	}
	if (isSmallScreen || isCoarsePointer) {
		return {
			baseDensity: MOBILE_FISH_DENSITY,
			zoneMultipliers: MOBILE_ZONE_DENSITY_MULTIPLIERS,
		};
	}
	return {
		baseDensity: DESKTOP_FISH_DENSITY,
...
const createSeaweedPatches = (count = SEAWEED_PATCH_COUNT) => {
	...
	return patches.sort((a, b) => a.leftValue - b.leftValue).map(({ leftValue, ...patch }) => patch);
};

const buildSchoolCreatures = (densityProfile) => {
	const baseDensity = densityProfile?.baseDensity ?? DESKTOP_FISH_DENSITY;
	const zoneMultipliers = densityProfile?.zoneMultipliers || DESKTOP_ZONE_DENSITY_MULTIPLIERS;

	return SCHOOL_BLUEPRINTS.flatMap((school) => {
		const rng = seededRng(hashString(`school-${school.id}`));
		const zoneMultiplier = zoneMultipliers[school.zone] ?? 1;
		const count = Math.max(1, Math.round(school.count * baseDensity * zoneMultiplier));
...
const buildCreatures = (asset, densityProfile) => [...buildSchoolCreatures(densityProfile), ...buildFeatureCreatures(asset)];
...
export default function OceanBackground() {
	...
	const zoneOpacitiesRef = useRef(getZoneOpacities(0));
	...
	useEffect(() => {
		...
		zoneOpacitiesRef.current = getZoneOpacities(depth);
	}, [depth]);
	...
	const fishDensityProfile = useMemo(() => getFishDensityProfile(), []);
	const creatures = useMemo(() => buildCreatures(asset, fishDensityProfile), [asset, fishDensityProfile]);
	const schoolCreatures = useMemo(() => creatures.filter((creature) => creature.kind === "fish"), [creatures]);
	const featureCreatures = useMemo(() => creatures.filter((creature) => creature.kind !== "fish"), [creatures]);
	const schoolFishRenderMap = useMemo(() => {
		return schoolCreatures.reduce((map, creature) => {
			const style = CANVAS_FISH_ZONE_STYLE[creature.zone] || CANVAS_FISH_ZONE_STYLE.sunlight;
			map.set(creature.id, {
				zone: creature.zone,
				size: creature.size,
				opacity: style.opacity,
				color: brightenHexColor(creature.color, style.brighten),
			});
			return map;
		}, new Map());
	}, [schoolCreatures]);
	...
	useEffect(() => {
		...
		const clearFishCanvas = () => {
			if (!canvasCtx || canvasWidth === 0 || canvasHeight === 0) return;
			canvasCtx.setTransform(1, 0, 0, 1, 0, 0);
			canvasCtx.clearRect(0, 0, canvasWidth, canvasHeight);
		};

		const renderSchoolFishCanvas = (fishFrame) => {
			if (!ensureFishCanvas()) return;
			if (!canvasCtx) return;
			...
			for (let i = 0; i < fishFrame.length; i += 1) {
				...
				if (!fish.isSchoolFish) continue;

				const metadata = schoolFishRenderMap.get(fish.id);
				if (!metadata) continue;

				const zoneOpacity = (zoneOpacityByZone[metadata.zone] ?? 1) * metadata.opacity;
				...
				canvasCtx.globalAlpha = zoneOpacity;
				canvasCtx.fillStyle = metadata.color;

				if (FISH_PATH_2D) {
					canvasCtx.fill(FISH_PATH_2D);
				} else {
					canvasCtx.beginPath();
					canvasCtx.ellipse(12, 12, 10.5, 5, 0, 0, Math.PI * 2);
					canvasCtx.fill();
				}
		...
		if (typeof Worker !== "undefined") {
			...
			const buildProfilerSnapshot = () => {
				...
				const avgRenderMs = profiler.renderFrames > 0 ? Number((profiler.renderMsTotal / profiler.renderFrames).toFixed(3)) : 0;
				...
				return {
					...
					schoolFishCount: schoolCreatures.length,
					fishCanvasMP: canvasWidth > 0 && canvasHeight > 0 ? Number(((canvasWidth * canvasHeight) / 1000000).toFixed(2)) : null,
	...
	return (
		...
		>
			...
			<canvas ref={fishCanvasRef} className="ocean-fish-canvas" aria-hidden="true" />

			{featureCreatures.map((creature) => {
				...
				return (
					<div
						...
						style={{
							...
							"--creature-facing":
								creature.species === "shark" ? (creature.direction === "left" ? 1 : -1) : creature.direction === "left" ? -1 : 1,
							"--creature-yaw": "0deg",
